...
import { validateMatrixCouncils, computePostingDormIssues } from '../util/matrixValidation.js';
...
const loadMatrixValidationContext = async () => {
  ...
  return { seniorCounselors, schools, dorms };
};
...
router.post('/validate-councils', async (req, res) => {
  try {
    ...
    const { seniorCounselors, schools, dorms } = await loadMatrixValidationContext();
    const validated = validateMatrixCouncils(councils, seniorCounselors, schools);
    const postingDormIssues = computePostingDormIssues(validated, dorms, seniorCounselors);
    return res.status(200).json({ councils: validated, postingDormIssues });
  } catch (err) {
