...
import { isAuthenticatedNonAdmin } from './_middleware.js';
...
router.get('/capabilities', isAuthenticatedNonAdmin, (req, res) => {
  return res.status(200).json({
    claudeStory: Boolean(process.env.ANTHROPIC_API_KEY),
  });
});

router.get('/files', isAuthenticatedNonAdmin, async (req, res) => {
  try {
...
router.post('/files', isAuthenticatedNonAdmin, async (req, res) => {
  try {
...
router.get('/files/:id', isAuthenticatedNonAdmin, async (req, res) => {
  try {
...
router.get('/files/:id/stories', isAuthenticatedNonAdmin, async (req, res) => {
  try {
...
router.post('/files/:id/stories/generate', isAuthenticatedNonAdmin, async (req, res) => {
  try {
    const file = await storage.model('RollCallCsvFile').getById(req.params.id);
...
router.put('/stories/:id', isAuthenticatedNonAdmin, async (req, res) => {
  try {
...
router.post('/powerpoint', isAuthenticatedNonAdmin, async (req, res) => {
  try {
