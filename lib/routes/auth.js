...
router.post('/login', async (req, res) => {
  try {
    ...
    if (username && password) {
      currentUser = await storage.model('SeniorCounselor').findOne({ username });
      if (!currentUser) {
        currentUser = await storage.model('User').findOne({ username });
      ...
      return res.status(200).json({ success: true, message: 'Login successful', user: currentUser });
    } else {
