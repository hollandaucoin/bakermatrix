 ...
 */
router.get('/', isSeniorCounselor, async (req, res) => {
  try {
 ...
 */
router.post('/', isSeniorCounselor, async (req, res) => {
  try {
    ...
    for (const assignment of assignments) {
      if (typeof assignment?.name !== 'string' || !assignment.name.trim()) {
        return res.status(400).json({ error: 'Each assignment must have a name' });
      }
    }
 ...
 */
router.put('/:submissionId', isSeniorCounselor, async (req, res) => {
  try {
 ...
 */
router.delete('/:submissionId', isSeniorCounselor, async (req, res) => {
try {
    const { submissionId } = req.params;
    if (!submissionId) {
    return res.status(400).json({ error: 'submissionId required' });
