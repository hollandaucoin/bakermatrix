 ...
 */
SeniorCounselor.statics.create = async function({ name, gender, committee, committeeLead, _jcPairing, _jcPairing2, _associatedSchool, _previousSchools, _previousPostingPartner } = {}) {
  try {
    // Validate the parameters
    if (typeof name !== 'string') { throw new HandledError('Valid name required', 400); }
    if (name.split(' ').length < 2) { throw new HandledError('Full name required', 400); }
    ...
    const lowercaseNameArray = name.toLowerCase().split(' ');
    const username = lowercaseNameArray[0].charAt(0) + lowercaseNameArray[1];
    const seniorCounselor = new storage.model('SeniorCounselor')({ name, username, admin: false, gender, committee, committeeLead, _jcPairing, _jcPairing2, _associatedSchool, _previousSchools, _previousPostingPartner });
 ...
 */
SeniorCounselor.methods.update = async function({ name, gender, committee, committeeLead, _jcPairing, _jcPairing2, _associatedSchool, _previousSchools, _previousPostingPartner, federalWay } = {}) {
  try {
    ...
    if (name) {
      ...
      if (name.split(' ').length < 2) { throw new HandledError('Full name required', 400); }
      this.name = name;
      this.username = name.toLowerCase().split(' ')[0].charAt(0) + name.toLowerCase().split(' ')[1];
    }
    if (gender) {
      if (typeof gender !== 'string') { throw new HandledError('Valid gender required', 400); }
      this.gender = gender;
